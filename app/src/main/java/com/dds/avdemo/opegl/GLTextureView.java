...
public class GLTextureView extends TextureView implements TextureView.SurfaceTextureListener {
    ...
    public void setBackgroundDrawable(Drawable background) {
        // add a empty method to avoid fc in android Q
    }
    ...
    public void setRenderer(GLSurfaceView.Renderer renderer) {
        ...
        mRenderer = renderer;
        mGLThread = new GLThread(mThisWeakRef, mPreservedWidth, mPreservedHeight);
        mGLThread.start();
    }
    ...
    public void requestRender() {
        if (mGLThread == null) {
            Log.w(TAG, "ignore requestRender, mGLThread is null");
            return;
        }
    ...
    public void onSurfaceTextureSizeChanged(SurfaceTexture surfaceTexture, int width, int height) {
        mPreservedWidth = width;
        mPreservedHeight = height;
    ...
    public interface EGLContextFactory {
        EGLContext createContext(EGL10 egl, EGLDisplay display, EGLConfig eglConfig, EGLContext shareContext);
    ...
    private class DefaultContextFactory implements EGLContextFactory {
        ...
        public EGLContext createContext(EGL10 egl, EGLDisplay display, EGLConfig config, EGLContext shareContext) {
            ...
            return egl.eglCreateContext(display, config,
                    shareContext == null ? EGL10.EGL_NO_CONTEXT : shareContext,
    ...
    private static class EglHelper {
        ...
        public void start() {
            ...
            if (view == null) {
                ...
                */
                EGLContext shareContext = view.mShareContextGetter == null ? null :
                                          view.mShareContextGetter.getShareContext();
                mEglContext = view.mEGLContextFactory.createContext(mEgl, mEglDisplay, mEglConfig, shareContext);
            }
    ...
    static class GLThread extends Thread {
        ...
        private void guardedRun() throws InterruptedException {
            ...
            try {
                        ...
                        while (true) {
                            ...
                            if (pausing) {
                                if (sGLThreadManager.shouldTerminateEGLWhenPausing()) {
                                    mEglHelper.finish();
                                    if (LOG_SURFACE) {
                                        Log.i(TAG, "terminating EGL because paused tid=" + getId());
                                    }
                                }
                            }
                ...
                Log.w(TAG, "render loop exit due to exception", e);
            } finally {
                /*
                 * clean-up everything...
                 */
        ...
        public void onPause() {
            synchronized (sGLThreadManager) {
                ...
                while ((! mExited) && (! mPaused)) {
                    if (LOG_PAUSE_RESUME) {
                        Log.i(UI_THREAD_TAG, "onPause waiting for mPaused.");
                    }
                    try {
                        sGLThreadManager.wait();
        ...
        public void onResume() {
            synchronized (sGLThreadManager) {
                ...
                while ((! mExited) && mPaused && (!mRenderComplete)) {
                    if (LOG_PAUSE_RESUME) {
                        Log.i(UI_THREAD_TAG, "onResume waiting for !mPaused.");
                    }
                    try {
                        sGLThreadManager.wait();
                    } catch (InterruptedException ex) {
        ...
        public void onWindowResize(int w, int h) {
            synchronized (sGLThreadManager) {
                ...
                while (! mExited && !mPaused && !mRenderComplete
                        && ableToDraw()) {
                    if (LOG_SURFACE) {
                        Log.i(UI_THREAD_TAG, "onWindowResize waiting for render complete from tid=" + getId());
                    }
                    try {
                        sGLThreadManager.wait();
                    } catch (InterruptedException ex) {
